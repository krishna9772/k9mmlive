...
<div {{ $attributes->merge(['class'=>'']) }}>
    ...
    <div class="mt-3 dark:text-white">
        @foreach ($matches as $match )
            <a target="{{$match->live_link? '_blank':'_self'}}" href="{{ $match->live_link ?: lang_route('frontend.sport.live-match',["slug"=>$match->slug])  }}" class="flex w-full px-1 py-2 mb-2 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
